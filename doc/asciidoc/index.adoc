
= Neo4j Connector for Apache Spark v{docs-version}
:toc:
:toclevels: 5
:sectids:
:sectlinks:
:sectnums:
:sectnumlevels: 5
:env-docs: true

ifdef::backend-html5[(C) {copyright}]

License: link:{common-license-page-uri}[Creative Commons 4.0]

[abstract]
--
This is the user guide for Neo4j Connector for Apache Spark version {docs-version}, authored by the Neo4j Labs Team.
--

The guide covers the following areas:

* <<_project_overview>> -- Project overview
* <<_quick_start>> -- Get Started Fast with the most Common Scenarios
* <<neo4j_causal_cluster>> -- Using with Neo4j Causal Cluster
* <<cloud>> -- Configuring a connection to a Confluent Cloud
* <<faq>> -- Neo4j Streams FAQ


include::overview/index.adoc[]

include::quickstart/index.adoc[]

include::org.neo4j.neo4j-cluster/index.adoc[]

include::cloud/index.adoc[]

include::faq/index.adoc[]